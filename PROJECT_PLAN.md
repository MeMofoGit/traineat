...
*UX feature. Banner no bloqueante con 3 opciones cuando hay un entreno sin completar.*
...
- ☐ **C5b.1** Detección de entrenos pendientes: comparar `plan.routines[phase][dayId]` (tiene ejercicios) contra `history` (no hay sesión completada ese día de la semana actual). Lógica en `useSchedule.js` o hook nuevo `usePendingWorkouts`.
- ☐ **C5b.2** Banner "Smart Suggestion" en `Training.jsx`, encima del día activo:
...
- ☐ **C5b.3** Almacenamiento de decisiones en `plan.weeklyDecisions: { [dayId]: 'skipped' | 'done_elsewhere' }`. Reset automático cada lunes (o al cambiar de semana según ISO).
- ☐ **C5b.4** Historial: cuando el usuario pulsa "Entrenar hoy" con una rutina de otro día, la sesión se registra en `history` con campo `originalDayId` para distinguir "hizo la rutina del lunes pero en martes".
...
> ⚠ **PENDIENTE DE ANÁLISIS**: el flujo de "Entrenar hoy" puede crear cascada — al hacer la rutina pendiente del lunes el martes, la rutina del martes pasa a ser la nueva "pendiente" para el miércoles. Hay que analizar y definir cómo manejar este escenario (¿se muestra un segundo banner? ¿se limita a 1 pendiente máximo? ¿se ofrece reagrupar el resto de la semana?). Iterar sobre este punto antes de implementar.
