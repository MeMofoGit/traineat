 ...
 *
 * Cadena de búsqueda actual (Fase 2):
 *   1. productCache/{barcode} en Firestore — lookups previos cacheados de por vida.
 *   2. API live de OpenFoodFacts.
 *   3. Si miss total → lanza BARCODE_NOT_FOUND (el cliente cae a OCR/manual).
 *
 * Cuando llegue Fase 4, el mirror nocturno `offProducts/{barcode}` se
 * antepondrá al productCache (paso 0) para servir sin tocar API live.
 *
...
export const lookupBarcode = onCall<LookupBarcodeRequest>(
  ...
  async (
    ...
    // --- 3. Validación server-side del resultado mapeado antes de cachear ---
